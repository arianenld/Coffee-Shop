...
class Customer(pygame.sprite.Sprite):
    ...
    def update(self):
        global totalSales, notif, seconds, amount, sales
        ...
        if self.exitQueue:
            if self.rect.x <= 400:
                ...
                if self.rect.y > 545:
                    ...
                    print(salesCount)
                    print("CUSTOMER COUNT:" +str(self.customerCtr))

        if self.spawnCust:
            customers = Customer()
            customers_.add(customers)
            allSprites.add(customers)
            self.spawnCust = False
...
allSprites.add(customers, counter)
customers_.add(customers)
...
while done:
    ...
    screen.blit(itemsDisplay, (667, 270))
